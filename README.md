...
## 路线图 / Roadmap：课表自动更新检测

> 当前状态：**第一期（手动检查更新）已实现初版代码，尚未做真机端到端验证**。以下记录设计思路，便于后续开发和 PR 讨论对齐。

现状：导入是一次性的——打开学校配置对应的登录页 → 抓取课表 → 解析后**总是新建一张课表**写入本地数据库。之后教务系统发生调课、换教室、换教师等变化，只能靠用户手动重新走一遍导入流程，且不会自动提示"哪里变了"。

计划分三期推进：

1. **第一期（手动检查更新 + 展示变更）**：复用现有的学校配置（`api/schoolList.json`）与 WebView 抓取/解析链路（`CourseParser`/`CourseParserXK`），新增一个"只解析不入库"的路径，把新抓取结果和上一次保存的快照做 diff，展示新增/删除/教室教师等字段变更，由用户确认后再写回数据库。这一步风险最低，先把"变更检测"这个核心能力跑通。
2. **第二期（前台节流自动检查）**：**不采用 OS 级后台任务**（Android WorkManager / iOS BGTaskScheduler 等）。原因：抓取链路依赖一个真实渲染并执行 JS 的 WebView 而非纯 HTTP 请求，iOS 上无界面后台执行历史上并不稳定，且 Android / iOS / 鸿蒙三端行为差异较大，投入产出比低。改为在 App 启动或切前台时判断"距上次检查是否超过用户设置的间隔"，达到阈值才静默触发第一期的检查流程。间隔默认**每天一次**，可由用户调整为"每次打开 / 每天 / 每 3 天 / 每周 / 手动"——课表变化频率本身很低，没有必要做成实时监控。若自动检查过程中遇到需要重新登录或验证码，第二期不做自动处理，只提示用户手动打开 App 检查。
3. **第三期（更远期，暂不评估细节）**：验证码自动识别（参考 [AutoCaptcha](https://github.com/treehey/AutoCaptcha) 的 Tesseract.js + 模板重排方案，以及 [nju-captcha-extension](https://github.com/yama-lei/nju-captcha-extension) 的小型 ONNX 模型方案，两者均为浏览器脚本，需要移植到 Dart 侧并针对验证码当前样式重新采集训练）、真正的 OS 后台任务、日历增量同步（修复导出到系统日历时因缺少事件 UID 导致的重复事件问题）、ICS 订阅链接等。这些需要额外的服务器组件或更高的工程投入，暂不作为近期目标。

...
| # | 任务 | 依赖后台方案成立？ |
| --- | --- | --- |
| 1 | 抽出 `CourseSyncService`，两个入口改为调用它 | 否 |
| 2 | 前台节流自动检查（打开 App / 切回前台时） | 否 |
| 3 | 验证完整登录链路在后台可行 | — |
| 4 | 接 WorkManager + 本地通知 + 失败保护 | 是 |
| 5 | 真机验证各厂商 ROM 表现 | 是 |

第 1 步有独立价值：目前 `ImportView` 与 `CheckUpdateView` **各写了一遍**"抓取 → 判学期 → 比对 → 覆盖 → 落库"，每次改动都要在两处同步。抽出服务层后重复消失，后台方案也有了现成地基。
